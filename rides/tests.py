...
class RideViewSetTests(TestCase):
    ...
    def test_check_edition_permission(self):
        user = UserFactory.create(email='fmajrox@gmail.com')
        vehicle = VehicleFactory.create(user=user)
        ride = RideWithPassengerFactory.create(
            **{'participation__decision': 'accepted', 'driver': user, 'vehicle': vehicle})

        response = self.client.get(f'/rides/{ride.ride_id}/check_edition_permissions/')

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(json.loads(response.content), {"full_permission": False})
